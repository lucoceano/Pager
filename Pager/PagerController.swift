...
enum PagerTabLocation: Int {
    case None = 0
    case Top = 1
    case Bottom = 2
}

enum PagerAnimation: Int{
    case None = 0
    case End = 1
    case During = 2
}

@objc protocol PagerDelegate: NSObjectProtocol {
...
@objc protocol PagerDataSource: NSObjectProtocol{
...
class PagerController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
    
    //public properties
	var contentViewBackgroundColor: UIColor?
    var indicatorColor: UIColor?
    var tabsViewBackgroundColor: UIColor?
    ...
    internal var tabsView: UIScrollView?// = UIScrollView()
    internal var pageViewController: UIPageViewController = UIPageViewController()
    ...
    func defaultSettings(){
        ...
        self.tabsViewBackgroundColor = UIColor.grayColor()
    ...
    func defaultSetup(){
        ...
        for(var i:Int = 0; i < self.tabCount; i++){
            ...
            var tapGestureRecognizer: UITapGestureRecognizer =  UITapGestureRecognizer(target: self, action: Selector("handleTapGesture:"))
    ...
    func layoutSubViews(){
        ...
        frame.origin.x = 0.0
        frame.origin.y = (self.tabLocation == PagerTabLocation.Top) ? topLayoutGuide : CGRectGetHeight(self.view.frame) - self.tabHeight
        ...
        frame.origin.x = 0.0
        frame.origin.y = (self.tabLocation == PagerTabLocation.Top) ? topLayoutGuide + CGRectGetHeight(self.tabsView!.frame) : topLayoutGuide
        frame.size.width = CGRectGetWidth(self.view.frame)
    ...
    @IBAction func handleTapGesture(sender: UITapGestureRecognizer){
		...
		let index:Int = self.tabs.find{ $0 as UIView? == tabView }!
    ...
    func selectTabAtIndex(index: Int, swipe: Bool){
        ...
        var previousIndex:Int = self.activeTabIndex
    ...
    func tabViewAtIndex(index: Int) -> TabView? {
        ...
        if (self.tabs[index] as UIView?) == nil{
            ...
            tabViewContent.autoresizingMask = UIViewAutoresizing.FlexibleHeight | UIViewAutoresizing.FlexibleWidth
    ...
    func viewControllerAtIndex(index: Int) -> UIViewController? {
        ...
        if (self.contents[index] as AnyObject?) == nil {
            ...
            if(self.dataSource!.respondsToSelector(Selector("controllerForTabAtIndex:pager:"))){
				...
				if(self.dataSource!.respondsToSelector(Selector("viewForTabAtIndex:pager:"))){
    ...
    func setActiveContentIndex(activeContentIndex: Int){
        ...
        if(activeContentIndex == self.activeContentIndex){
            ...
            self.pageViewController.setViewControllers([viewController!], direction: UIPageViewControllerNavigationDirection.Forward, animated: false, completion:{ (completed:Bool) -> Void in
            ...
            var direction: UIPageViewControllerNavigationDirection = (activeContentIndex < self.activeContentIndex) ? UIPageViewControllerNavigationDirection.Reverse : UIPageViewControllerNavigationDirection.Forward
            
            self.pageViewController.setViewControllers([viewController!], direction: direction, animated: true, completion:{ (completed:Bool) -> Void in
...
//UIScrollViewDelegate, Responding to Scrolling and Dragging
	func scrollViewDidScroll(scrollView: UIScrollView) {
		...
		var tabView:UIView  = self.tabViewAtIndex(self.activeTabIndex)!
		
		if(!self.animatingToTab){
			
			// Get the related tab view position
			var frame:CGRect = tabView.frame
			var movedRatio: CGFloat  = (scrollView.contentOffset.x / CGRectGetWidth(scrollView.frame)) - 1
		...
		var updateIndicator = { (newX:CGFloat) -> Void in
			rect.origin.x = newX
		...
		var distance:CGFloat = tabView.frame.size.width
